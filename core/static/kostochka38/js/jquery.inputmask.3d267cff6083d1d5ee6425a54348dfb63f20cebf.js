            ...
            this.mask = function (el) {
                ...
                if (androidchrome) {
                    $el.bind("input.inputmask", function (e) {
                        ...
                        var chromeValueAfterInput = getActiveBuffer().join('');
                        if (chromeValueOnInput.length == $input.val().length + 1 && chromeValueAfterInput == chromeValueOnInput) {
                            var idx;
                            for (idx = 0; idx < $input.val().length; idx++) {
                                if (chromeValueOnInput[idx] != $input.val()[idx]) {
                                    break;
                                }
                            }
                            if (idx < $input.val().length) {
                                idx -= 1;
                                var str = chromeValueOnInput.substr(0, idx) + opts.placeholder + chromeValueOnInput.substr(idx + 1, chromeValueOnInput.length);
                                checkVal(input, false, false, str);
                            }
                        }
                        writeBuffer(input, getActiveBuffer());
                        if (isComplete(getActiveBuffer()) === true)
